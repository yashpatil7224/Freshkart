...
async function sendOTPViaEmailJS(toEmail, otpCode, toName = 'Valued Customer') {
    const templateParams = {
        ...
        code: otpCode,
        passcode: otpCode,
        subject: 'Your FreshKart Email Verification Code (OTP)',
        message: `Your verification code is ${otpCode}. Valid for 10 minutes.`
    };

    const pKey = EMAILJS_CONFIG.publicKey || window.EMAILJS_PUBLIC_KEY;
    if (!pKey) {
        console.warn('EmailJS Public Key not set. Set window.EMAILJS_PUBLIC_KEY in index.html');
        return { success: false, reason: 'missing_key' };
    }

    try {
        if (window.emailjs) {
            await emailjs.send(EMAILJS_CONFIG.serviceId, EMAILJS_CONFIG.otpTemplateId, templateParams, pKey);
            return { success: true, method: 'sdk' };
        }
    } catch (err) {
        console.warn('EmailJS SDK send error, trying REST API fallback:', err);
    }
    ...
    try {
        const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
            ...
            body: JSON.stringify({
                ...
                template_id: EMAILJS_CONFIG.otpTemplateId,
                user_id: pKey,
...
// 2. Send Parcel Delivery Invoice Bill via EmailJS
async function sendBillViaEmailJS(toEmail, toName, orderObj) {
    if (!toEmail || !toEmail.includes('@')) return { success: false };

    const itemsSummary = (orderObj.items || []).map(i => `${i.title} (${i.qty || 1} x ₹${i.price})`).join(', ');
    const deliveryAddr = orderObj.delivery ? orderObj.delivery.address : 'Address provided at checkout';
    const pKey = EMAILJS_CONFIG.publicKey || window.EMAILJS_PUBLIC_KEY;

    if (!pKey) return { success: false, reason: 'missing_key' };

    const templateParams = {
        to_email: toEmail,
        email: toEmail,
        user_email: toEmail,
        to_name: toName || (orderObj.delivery ? orderObj.delivery.name : 'Valued Customer'),
        name: toName || (orderObj.delivery ? orderObj.delivery.name : 'Valued Customer'),
        order_id: orderObj.id,
        order_date: orderObj.date || new Date().toLocaleDateString(),
        order_status: orderObj.status || 'Placed / Processing',
        items_summary: itemsSummary,
        delivery_address: deliveryAddr,
        total_amount: `₹${parseFloat(orderObj.total || 0).toFixed(2)}`,
        payment_mode: orderObj.delivery ? (orderObj.delivery.payment || 'COD').toUpperCase() : 'COD'
    };

    try {
        if (window.emailjs) {
            await emailjs.send(EMAILJS_CONFIG.serviceId, EMAILJS_CONFIG.billTemplateId, templateParams, pKey);
            return { success: true, method: 'sdk' };
        }
    } catch (err) {}
    ...
    try {
        const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
            ...
            body: JSON.stringify({
                service_id: EMAILJS_CONFIG.serviceId,
                template_id: EMAILJS_CONFIG.billTemplateId,
                user_id: pKey,
                template_params: templateParams
            })
        });
...
window.sendRegistrationOTP = async function() {
    ...
    try {
        ...
        if (sendBtn) {
            sendBtn.disabled = false;
            sendBtn.innerHTML = 'Sent OTP <i class="fa-solid fa-check"></i>';
        }
    }
};
...
window.sendForgotPasswordOTP = async function() {
    ...
    try {
        ...
        if (response && response.ok) {
            ...
            if (data.otp_preview) {
                sendOTPViaEmailJS(email, data.otp_preview);
            }
        ...
        sendOTPViaEmailJS(email, demoCode);
        showToast(`📩 Verification code sent to <strong>${email}</strong> via EmailJS! Check your email inbox.`, 'success');
        if (notice) {
            notice.classList.remove('hidden');
            notice.innerHTML = `<i class="fa-solid fa-circle-check"></i> Reset code sent to <strong>${email}</strong>`;
